...
import { ChevronUp, ChevronDown, PlusCircle, XCircle, FileDown, Folder, Copy, Trash2, Lock, Unlock, Clipboard } from 'lucide-react';
...
const loadScript = (src, callback) => {
    ...
    script.onerror = () => {
        ...
        alert(`Failed to load export library. Please check your internet connection and try again.`);
    };
...
const App = () => {
    ...
    const [pasteModalInfo, setPasteModalInfo] = useState({ isOpen: false, tractId: null });
    ...
    const updateTract = useCallback((tractId, field, value) => setProjectState(p => ({...p, tracts: p.tracts.map(t => t.id === tractId ? { ...t, [field]: value } : t)})), [activeProject]);
    const toggleTract = useCallback((tractId) => setProjectState(p => ({...p, tracts: p.tracts.map(t => t.id === tractId ? { ...t, isExpanded: !t.isExpanded } : t)})), [activeProject]);
    ...
    const removeOwner = useCallback((tractId, ownerId) => setProjectState(p => ({...p, tracts: p.tracts.map(t => t.id === tractId ? { ...t, owners: t.owners.length > 1 ? t.owners.filter(o => o.id !== ownerId) : t.owners } : t)})), [activeProject]);
    
    const updateOwner = useCallback((tractId, ownerId, field, value) => {
    ...
    const exportToXLSX = useCallback(() => {
        ...
        if (typeof XLSX === 'undefined') {
            loadScript("https://cdn.sheetjs.com/xlsx-0.20.1/package/dist/xlsx.full.min.js", performExport);
        } else {
            performExport();
        }
                            ...
                            {Object.values(projects).map(proj => (
                                <div key={proj.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-slate-100">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <button onClick={() => handleDuplicateProject(proj.id)} className="text-sm bg-gray-500 text-white py-1 px-3 rounded-md hover:bg-gray-600"><Copy size={14}/></button>
                                        <button onClick={() => handleDeleteProject(proj.id)} className="text-slate-400 hover:text-red-600"><Trash2 size={18}/></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={toggleUnitAcresMode} className="p-2 rounded-md hover:bg-slate-200 text-slate-500">
                                    {isUnitAcresManual ? <Unlock size={18} /> : <Lock size={18} />}
                                </button>
                            </div>
                        </div>
            ...
            <div className="p-4 sm:px-6 lg:px-8 flex-shrink-0 border-t border-slate-200 mt-4">
                 <div className="flex flex-col sm:flex-row gap-4">
                    <button onClick={addTract} className="flex items-center justify-center gap-2 bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-blue-700 transition-all duration-200 w-full sm:w-auto">
                        <PlusCircle size={18} /> Add Tract
                    </button>
                    <button onClick={() => setIsExportModalOpen(true)} className="flex items-center justify-center gap-2 bg-green-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md hover:bg-green-700 transition-all duration-200 w-full sm:w-auto">
                        <FileDown size={18} /> Export to XLSX
                    </button>
                </div>
            </div>
        </div>
    ...
    <div className="border-b border-slate-200 last:border-b-0">
        <div className="grid grid-cols-12 gap-4 p-3 bg-slate-50 items-center">
            <div className="col-span-5 flex items-center">
                <button onClick={() => onToggleTract(tract.id)} className="mr-2 text-slate-500 hover:text-slate-800">{tract.isExpanded ? <ChevronUp size={20} /> : <ChevronDown size={20} />}</button>
            ...
            <div className="col-span-2 flex justify-end gap-2">
                <button onClick={() => onDuplicateTract(tract.id)} className="text-blue-500 hover:text-blue-700 transition-colors"><Copy size={20} /></button>
                <button onClick={() => onRemoveTract(tract.id)} className="text-red-500 hover:text-red-700 transition-colors"><XCircle size={20} /></button>
            </div>
        </div>
        {tract.isExpanded && (
            <div>
                ...
                <div className="p-2 flex justify-start pl-4 gap-4">
                    <button onClick={() => onAddOwner(tract.id)} className="flex items-center gap-1 text-sm text-blue-600 hover:text-blue-800 font-semibold"><PlusCircle size={16} /> Add Owner Interest</button>
                    <button onClick={() => onOpenPasteModal(tract.id)} className="flex items-center gap-1 text-sm text-green-600 hover:text-green-800 font-semibold"><Clipboard size={16} /> Paste from Sheet</button>
                </div>
            </div>
        )}
    </div>
    ...
    return (
        <div className="grid grid-cols-12 gap-4 p-3 pl-4 items-center border-t border-slate-100 hover:bg-blue-50/30 group">
            ...
            <div className="col-span-2 flex items-center justify-end">
                ...
                <button onClick={() => onRemoveOwner(tract.id, owner.id)} className="ml-2 text-slate-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><XCircle size={16} /></button>
            </div>
        </div>
    );
};
...
const PasteDataModal = ({ tract, onPaste, onClose }) => {
    ...
    const [columns, setColumns] = useState([{ id: 'name', name: 'Owner', type: 'text' }]);
